...
for (const [en, key] of pairs) {
  ...
  // just text directly (for headers without whitespace if above doesn't work)
  // wait the RegEX with spaces should match
}

// Special cases manually
code = code.replace('Trace</button>', `{t('trace')}</button>`);
code = code.replace('Tracing...</button>', `{t('tracing')}</button>`);
